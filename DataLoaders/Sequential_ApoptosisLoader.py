...
class DataLoader(object):
    ...
    def __init__(self, cfg):
        ...
        self.data_path = './data/sequential_data_' + str(cfg.height) + '.h5'
    ...
    def next_batch(self, start=None, end=None, mode='train'):
        if mode == 'train':
            ...
            x, y = self.reshape_(x, y)
            if self.cfg.data_augment:
                x = random_rotation_2d(x, max_angle=self.cfg.max_angle)
        elif mode == 'valid':
    ...
    def reshape_(self, x, y):
        return x.reshape(self.cfg.batch_size*self.cfg.max_time, self.cfg.height, self.cfg.width, self.cfg.channel), \
               y.reshape(self.cfg.batch_size*self.cfg.max_time, self.cfg.num_cls)
