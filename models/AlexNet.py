...
class AlexNet(BaseModel):
    ...
    def build_network(self, x):
        ...
        with tf.variable_scope('CapsNet'):
            ...
            net = lrn(
                relu(conv_layer(net, kernel_size=5, stride=2, num_filters=256, trainable=self.conf.trainable,
            ...
            self.net_grad = relu(
                conv_layer(net, kernel_size=3, stride=1, num_filters=256, trainable=self.conf.trainable,
                           add_reg=self.conf.L2_reg, layer_name='CONV5'))
            net = max_pool(self.net_grad, pool_size=3, stride=2, padding='SAME', name='MaxPool3')
