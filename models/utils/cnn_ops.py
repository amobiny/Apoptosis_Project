...
def flatten_layer(layer):
    """Flattens the output of the convolutional layer
    to prepare it to be fed in to the fully-connected layer"""
...
def fc_layer(bottom, out_dim, name, is_train=True, batch_norm=False,
             add_reg=False, use_relu=True):
    """Create a fully connected layer"""
    in_dim = bottom.get_shape()[1]
    with tf.variable_scope(name):
        weights = weight_variable(shape=[in_dim, out_dim])
        tf.summary.histogram('W', weights)
